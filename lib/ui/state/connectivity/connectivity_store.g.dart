// GENERATED CODE - DO NOT MODIFY BY HAND

part of 'connectivity_store.dart';

// **************************************************************************
// StoreGenerator
// **************************************************************************

// ignore_for_file: non_constant_identifier_names, unnecessary_brace_in_string_interps, unnecessary_lambdas, prefer_expression_function_bodies, lines_longer_than_80_chars, avoid_as, avoid_annotating_with_dynamic, no_leading_underscores_for_local_identifiers

mixin _$ConnectivityStore on ConnectivityStoreBase, Store {
  late final _$statusAtom =
      Atom(name: 'ConnectivityStoreBase.status', context: context);

  @override
  ConnectivityStatuses get status {
    _$statusAtom.reportRead();
    return super.status;
  }

  @override
  set status(ConnectivityStatuses value) {
    _$statusAtom.reportWrite(value, super.status, () {
      super.status = value;
    });
  }

  late final _$ConnectivityStoreBaseActionController =
      ActionController(name: 'ConnectivityStoreBase', context: context);

  @override
  void connected() {
    final _$actionInfo = _$ConnectivityStoreBaseActionController.startAction(
        name: 'ConnectivityStoreBase.connected');
    try {
      return super.connected();
    } finally {
      _$ConnectivityStoreBaseActionController.endAction(_$actionInfo);
    }
  }

  @override
  void disconnected() {
    final _$actionInfo = _$ConnectivityStoreBaseActionController.startAction(
        name: 'ConnectivityStoreBase.disconnected');
    try {
      return super.disconnected();
    } finally {
      _$ConnectivityStoreBaseActionController.endAction(_$actionInfo);
    }
  }

  @override
  String toString() {
    return '''
status: ${status}
    ''';
  }
}
